...
# zap/burp 추가 근거(2026-07-17): correlation-engine S28(알려진 스캐너 툴 User-Agent
# 탐지) 검증 중 OWASP ZAP(zap-full-scan.py) active scan을 실제로 돌려봤더니, 이
# 목록에 sqlmap/nikto/nmap 같은 옛 CLI 툴만 있고 지금 가장 널리 쓰이는 웹 취약점
# 스캐너인 ZAP/Burp Suite가 빠져 있어서 S28이 전혀 발화하지 않았다(WAF 단계에서부터
# bad_bot 이벤트 자체가 안 만들어짐) - 실전에서 공격자가 이 두 도구를 쓰면 그대로
# 탐지를 우회한다는 뜻이라 목록에 추가.
BAD_BOT_USER_AGENTS = ["sqlmap", "nikto", "nmap", "masscan", "acunetix", "curl/7.0", "zap", "burp"]
...
# IP별 최근 (시각, User-Agent) 기록 (UA 로테이션 탐지용) - Rate Limiting과 별개
# 저장소를 쓴다: rate limit은 "요청 수"만 세지만 이건 "distinct User-Agent 수"를
# 세야 해서 각 요청의 UA 값 자체를 들고 있어야 한다.
_user_agents_by_ip: Dict[str, Deque[Tuple[float, str]]] = defaultdict(deque)
...
def record_user_agent_rotation(ip: str, user_agent: str) -> bool:
    """같은 IP가 ua_rotation_window_seconds 안에 서로 다른(distinct) User-Agent를
    ua_rotation_distinct_threshold개 이상 쓰면 True. 정상 브라우저는 세션 내내 UA가
    고정되므로(리다이렉트/재시도에도 안 바뀜), 이 자체가 문자열 내용과 무관하게
    핑거프린팅 회피(스캐너가 매 요청마다 UA를 바꿔 탐지를 우회하려는 시도)의
    신호다. 빈 UA는 카운트에서 제외 - curl/서버간 호출처럼 원래 UA가 없는 정상
    트래픽까지 "매번 새로운 값"으로 잡히면 오탐이 나기 때문."""
    if not user_agent:
        return False

    now = time.time()
    history = _user_agents_by_ip[ip]

    while history and now - history[0][0] > settings.ua_rotation_window_seconds:
        history.popleft()

    history.append((now, user_agent))
    distinct_uas = {ua for _, ua in history}
    return len(distinct_uas) >= settings.ua_rotation_distinct_threshold
...
def _log_ua_rotation(ip: str, path: str, user_agent: str) -> None:
    save_log(
        WafAlert(
            source_ip=ip,
            attack_type=AttackType.UA_ROTATION,
            target_endpoint=path,
            http_method="",
            payload_snippet=f"User-Agent: {user_agent}"[:200],
            user_agent=user_agent,
...
class GatewayMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # 1-2) User-Agent 로테이션 탐지 — bad_bot이 문자열로 못 잡는 회피형 스캐너용
        #      보강(S28 재료, 2026-07-18). bad_bot과 별개 신호라 bad_bot에 걸렸든
        #      아니든 항상 기록한다.
